...
<jsp:useBean id="c" class="simple_add_edit_online_application.ListApplications" scope="application" />    
...
<table border='2'>
			...
			<c:forEach items="${application}" var="app">
				<tr>
					...
					<td><fmt:formatDate pattern="M/d/yyyy" value="${app.applicationReceived}"></fmt:formatDate></td>
					<%-- <td><c:out value="${app.applicationReceived}"></c:out></td> --%>
					<td><c:out value="${app.gpa}"></c:out></td>
					<td><c:out value="${app.status}"></c:out></td>
					<td><a href = "EditApplication?id=${app.id}&&applicant=${app.applicant}&&apprec=${app.applicationReceived}&&gpa=${app.gpa}&&status=${app.status}">Edit</a></td>
			</tr>
